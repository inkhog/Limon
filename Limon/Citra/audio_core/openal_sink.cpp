...
OpenALSink::OpenALSink(std::string device_name) : impl(std::make_unique<Impl>()) {
    ...
    alBufferCallbackSOFT(impl->buffer, AL_FORMAT_STEREO16, native_sample_rate, reinterpret_cast<ALBUFFERCALLBACKTYPESOFT>(&Impl::Callback),
                         impl.get());
    if (alGetError() != AL_NO_ERROR) {
    ...
    alSourcef(impl->source, AL_MAX_GAIN, 3.0);
    alSourcef(impl->source, AL_GAIN, 3.0);
